...
sealed interface WriteScreenActions {
    data class NavigateToMainPlayer(val uri: String): WriteScreenActions
                ...
                .collectLatest { result ->
                    when(result) {
                        ...
                        is Resource.Success -> {
                            ...
                            textSavior.save(text)
                            _uiActions.emit(WriteScreenActions.NavigateToMainPlayer(file.path))
                        }
                        is Resource.Error -> {
                            updateIsLoading(false)
